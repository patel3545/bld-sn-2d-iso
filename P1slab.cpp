...
SOLVE P1 EQUATIONS USING PETSC
...
int main(int argc, char **args)
{
		...
		Vec soln, srcP1;// For any system that solves one matrix, we only have two
		...
		PetscInt	i, j, itc, n_elem = 32;

		// REAL SCALARS

		PetscReal tol = 1e-8, norm, dx = L/n_elem;
		...
		// SOLVER CONTEXT AND PRECODITIONER CONTEXT
		 ...
		 ierr = VecDuplicate(soln, &srcP1); CHKERRQ(ierr);
		 ...
		 for(i = 0; i<n_elem; i++) {
			 ...
			 VecSetValue(srcP1, j, add, INSERT_VALUES);
		...
		VecAssemblyBegin(srcP1);
		VecAssemblyEnd(srcP1);

		// OUTPUT srcP1 (to terminal) VECTOR AND VERIFY

		ierr = PetscPrintf(PETSC_COMM_SELF, "srcP1 vector is:: \n"); CHKERRQ(ierr);
		ierr = VecView(srcP1, PETSC_VIEWER_STDOUT_SELF); CHKERRQ(ierr);
		...
		// Set linear solver for this problem
		...
		ierr = KSPSolve(ksp,srcP1,soln);CHKERRQ(ierr);
